...
function complexConvert(data) {
  ...
  for (let item of iterator) {
    if (typeof obj[item] === "number") obj[item] += 1;
  }

  return obj;

  //------------------------------------------------------------------------
  // вариант с заходом во вложенные объекты
  // const obj = {};
...
function searchParamsURL(url) {
  ...
  address.searchParams.forEach((value, key) => map.set(key, value));
...
function manipulateSearchParams(paramsObj, newUrl) {
  ...
  for (let item of keys) {
    url.searchParams.set(item, paramsObj[item]);
  }
...
function checkURLParameters(url, params) {
  ...
  for (let item of params) {
    if (urlObj.searchParams.has(item)) obj[item] = true;
    else obj[item] = false;
  }
...
function getURLValues(url) {
  ...
  //   result[result.length] = urlObj.searchParams.getAll(key);
  // }
  //-----------------------------------------------------------------------------

  for (let key of arrKeys) {
    const arr = urlObj.searchParams.getAll(key);
